...
static bool gui_prompt_filename(
    const char *prompt,
    char *path,
    uint8_t capacity)
{
...
static bool gui_capture(gui_state_t *state)
{
    state->captured = NULL;
    if (
        state->xms_ready && state->xms.size_bytes >= state->image_size && uf_image_store_init(&state->image_store, &state->runtime.hardware, state->xms.linear_base, state->xms.size_bytes) && uf_image_store_capture(&state->image_store, &state->runtime.flash, state->image_size))
    {
...
{
    gui_menu_item_t items[] = {
        {82, 115, 10, false, gui_unavailable},
...
cleanup:
    ...
    if (
        state.rom_enabled && !uf_runtime_set_rom_enabled(&state.runtime, false))
    {
        result = 1;
    }
    if (
        state.runtime_ready && !uf_runtime_shutdown(&state.runtime))
    {
        result = 1;
    }
