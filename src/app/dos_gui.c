...
static uf_bool_t gui_monochrome = UF_FALSE;

static const char *message(uf_message_id_t id)
{
    return uf_language_get(&uf_language_english, id);
}

static void gui_cursor(uf_bool_t visible)
{
...
{
    if (gui_monochrome) {
        gui_attribute = background == GUI_BLACK
            ? (foreground == GUI_GREEN
                ? UINT8_C(0x07)
                : UINT8_C(0x0F))
            : UINT8_C(0x70);
        return;
    }
    gui_attribute = (uint8_t)(
        (background << 4) | (foreground & UINT8_C(0x0F))
    );
}
...
static void gui_center(uint8_t row, const char *text)
{
    ...
    uint8_t column = length < 78
        ? (uint8_t)((80U - length) / 2U)
        : 1;
...
static void gui_draw_header(void)
{
    ...
    gui_write_at(
        9, 4,
        "Maintained by Ondrej Zary (Rainbow Software) until 2005"
    );
}

static void format_organisation(
...
)
{
    ...
    case UF_FLASH_BEHAVIOR_SECTOR:
        snprintf(output, output_size, "sectored (%lu bytes)",
            (unsigned long)chip->capacity_bytes);
        break;
    case UF_FLASH_BEHAVIOR_PAGE:
        snprintf(output, output_size, "%lu pages of %u bytes",
            (unsigned long)(
                chip->capacity_bytes / chip->page_size_bytes
            ),
            (unsigned int)chip->page_size_bytes);
        break;
    case UF_FLASH_BEHAVIOR_BULK_ERASE:
        snprintf(output, output_size, "bulk erase (%lu bytes)",
            (unsigned long)chip->capacity_bytes);
        break;
    default:
        snprintf(output, output_size, "%lu sectors of %u bytes",
            (unsigned long)(
                chip->capacity_bytes / chip->page_size_bytes
            ),
            (unsigned int)chip->page_size_bytes);
...
static void gui_draw_information(gui_state_t *state)
{
    ...
    snprintf(line, sizeof(line), "%s %s",
        state->runtime.flash.manufacturer->name,
        state->runtime.flash.chip->name);
    gui_puts(line);
    ...
    gui_clear_line(23);
    snprintf(line, sizeof(line), "%s%luK XMS%s",
        message(68),
        state->xms_ready
            ? (unsigned long)(state->xms.size_bytes >> 10)
            : 0UL,
    ...
    snprintf(line, sizeof(line), "%s%08lX",
        message(69),
        (unsigned long)state->runtime.flash.rom_base);
    gui_write_at(1, 24, line);
}

static uf_bool_t gui_prompt_filename(
    const char *prompt,
    char *path,
    uint8_t capacity
)
{
    ...
    gui_cursor(UF_FALSE);
    gui_clear_line(20);
    return length != 0 ? UF_TRUE : UF_FALSE;
}

static void gui_show_operation_result(
    gui_state_t *state,
    uf_read_result_t result,
    uf_message_id_t success_id
)
{
    gui_color(
        result == UF_READ_RESULT_OK ? GUI_LIGHT_GREEN : GUI_LIGHT_RED,
        GUI_BLACK
    );
    gui_clear_line(20);
    gui_center(
        20,
        result == UF_READ_RESULT_OK
            ? message(success_id)
...
static void gui_save_bootblock(gui_state_t *state)
{
    ...
    if (state->image_size < UINT32_C(8192)) {
        gui_show_operation_result(
            state, UF_READ_RESULT_INVALID_ARGUMENT, 143
        );
        return;
    }
    ...
    result = uf_read_workflow_dump_range(
        &state->runtime.flash,
        state->captured,
...
static void gui_save_cmos(gui_state_t *state)
{
    ...
    if (!gui_prompt_filename(
        "CMOS data file name: ", path, sizeof(path)
    )) {
        return;
    }
    gui_show_operation_result(
        ...
        uf_cmos_save_file(
            ...
            state->cmos_last_index,
            path
        ) ? UF_READ_RESULT_OK : UF_READ_RESULT_FILE_IO,
        145
    );
}

static uf_bool_t gui_capture(gui_state_t *state)
{
    state->captured = NULL;
    if (
        state->xms_ready
        && state->xms.size_bytes >= state->image_size
        && uf_image_store_init(
            &state->image_store,
            &state->runtime.hardware,
...
static void gui_quit(gui_state_t *state)
{
    state->done = UF_TRUE;
}

static void gui_draw_item(
...
)
{
    ...
    gui_color(
        item->enabled ? GUI_WHITE : GUI_LIGHT_GRAY,
        selected ? GUI_RED : GUI_BLACK
    );
...
static void gui_run_menu_items(
    gui_state_t *state,
...
)
{
    ...
    while (!state->done && !finished) {
        ...
        gui_color(
            items[selected].enabled ? GUI_LIGHT_GREEN : GUI_GREEN,
            GUI_BLACK
        );
        gui_clear_line(25);
        ...
        case 0:
            switch (getch()) {
            case 72:
                selected = uf_menu_move_selection(
                    selected, count, UF_MENU_MOVE_UP
                );
                break;
            case 80:
                selected = uf_menu_move_selection(
                    selected, count, UF_MENU_MOVE_DOWN
                );
                break;
            case 71:
                selected = uf_menu_move_selection(
                    selected, count, UF_MENU_MOVE_HOME
                );
                break;
            case 79:
                selected = uf_menu_move_selection(
                    selected, count, UF_MENU_MOVE_END
                );
                break;
            default:
                break;
...
{
    gui_menu_item_t items[] = {
        { 80, 118, 10, UF_TRUE, gui_save_cmos },
        { 81, 119, 11, UF_FALSE, gui_unavailable },
        { 88, 120, 12, UF_FALSE, gui_unavailable },
        { 89, 121, 13, UF_FALSE, gui_unavailable },
        { 79, 130, 15, UF_TRUE, NULL }
    };
...
{
    gui_menu_item_t items[] = {
        { 82, 115, 10, UF_FALSE, gui_unavailable },
        { 83, 116, 11, UF_TRUE, gui_save_bootblock },
        { 84, 117, 12, UF_FALSE, gui_unavailable },
        { 140, 141, 13, UF_TRUE, gui_compare },
        { 79, 130, 15, UF_TRUE, NULL }
    };
...
{
    gui_menu_item_t items[] = {
        { 85, 122, 10, UF_TRUE, gui_dump },
        { 86, 123, 11, UF_FALSE, gui_unavailable },
        { 87, 124, 12, UF_FALSE, gui_unavailable },
        { 100, 125, 13, UF_TRUE, gui_redetect },
        { 101, 126, 14, UF_TRUE, gui_cmos_menu },
        { 91, 128, 15, UF_TRUE, gui_advanced_menu },
        { 93, 129, 17, UF_TRUE, gui_quit }
    };
...
int uf_gui_run(uf_phys_addr_t rom_base, uf_bool_t monochrome)
{
    ...
    state.runtime_ready = UF_TRUE;
    if (!uf_runtime_set_rom_enabled(&state.runtime, UF_TRUE)) {
        fputs("Could not enable access to the system ROM.\r\n", stderr);
        goto cleanup;
    }
    state.rom_enabled = UF_TRUE;
    if (!uf_flash_service_detect(&state.runtime.flash)) {
        fputs("Flash ROM chip was not identified.\r\n", stderr);
        goto cleanup;
    }
    state.image_size = state.runtime.flash.chip->capacity_bytes;
    if (!uf_cmos_detect_last_index(
        &state.runtime.hardware, &state.cmos_last_index
    )) {
...
cleanup:
    ...
    if (
        state.rom_enabled
        && !uf_runtime_set_rom_enabled(&state.runtime, UF_FALSE)
    ) {
        result = 1;
    }
    if (
        state.runtime_ready
        && !uf_runtime_shutdown(&state.runtime)
    ) {
        result = 1;
    }
