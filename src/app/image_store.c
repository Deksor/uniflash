...
uf_bool_t uf_image_store_capture(
    uf_image_store_t *store,
...
)
{
    ...
    for (offset = 0; offset < image_size_bytes; ++offset) {
        if (
            !uf_flash_service_read_byte(flash, offset, &value)
            || !store->hardware->phys_write8(
...
)
{
    if (
        ...
        || offset > store->image_size_bytes
        || size_bytes > store->image_size_bytes - offset
    ) {
        return UF_FALSE;
    }
