...
static bool read_chunk(uf_flash_service_t *flash,
    ...
    for (uint16_t index = 0; index < size; ++index) {
        if (!uf_flash_service_read_byte(flash, offset + index, &buffer[index])) {
            return false;
        }
    }
    return true;
...
static uf_read_result_t validate_arguments(uf_flash_service_t *flash,
    const uf_image_store_t *store,
    uf_rom_size_t image_size_bytes,
    const char *path,
    uint8_t *buffer,
    uint16_t buffer_size) {
    if (flash == NULL || image_size_bytes == 0 || path == NULL || path[0] == '\0' || buffer == NULL ||
        buffer_size == 0 || (store != NULL && store->image_size_bytes != image_size_bytes)) {
        return UF_READ_RESULT_INVALID_ARGUMENT;
    }
    return UF_READ_RESULT_OK;
}

uf_read_result_t uf_read_workflow_dump(uf_flash_service_t *flash,
    const uf_image_store_t *store,
    uf_rom_size_t image_size_bytes,
    const char *path,
    uint8_t *buffer,
    uint16_t buffer_size) {
    return uf_read_workflow_dump_range(flash, store, 0, image_size_bytes, path, buffer, buffer_size);
}

uf_read_result_t uf_read_workflow_dump_range(uf_flash_service_t *flash,
    const uf_image_store_t *store,
    ...
    uint8_t *buffer,
    uint16_t buffer_size) {
    if (flash == NULL || size_bytes == 0 || path == NULL || path[0] == '\0' || buffer == NULL || buffer_size == 0 ||
        image_offset > UINT32_MAX - size_bytes ||
        (store != NULL &&
            (image_offset > store->image_size_bytes || size_bytes > store->image_size_bytes - image_offset))) {
        return UF_READ_RESULT_INVALID_ARGUMENT;
    }
    ...
    while (offset < size_bytes) {
        ...
        uint16_t chunk = remaining < buffer_size ? (uint16_t)remaining : buffer_size;

        if (!read_chunk(flash, store, image_offset + offset, buffer, chunk)) {
            fclose(file);
...
uf_read_result_t uf_read_workflow_compare(uf_flash_service_t *flash,
    const uf_image_store_t *store,
    uf_rom_size_t image_size_bytes,
    const char *path,
    uint8_t *buffer,
    uint16_t buffer_size) {
    uf_read_result_t validation = validate_arguments(flash, store, image_size_bytes, path, buffer, buffer_size);
    ...
    while (offset < image_size_bytes) {
        ...
        for (uint16_t index = 0; index < chunk; ++index) {
            uint8_t value;
            if (store != NULL && store->image_size_bytes != 0) {
                if (!uf_image_store_read(store, offset + index, &value, 1)) {
                    fclose(file);
                    return UF_READ_RESULT_FLASH_IO;
                }
            } else if (!uf_flash_service_read_byte(flash, offset + index, &value)) {
                fclose(file);
                return UF_READ_RESULT_FLASH_IO;
            }
