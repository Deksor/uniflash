...
{
    if (
        flash == NULL || image_size_bytes == 0 || path == NULL || path[0] == '\0' || buffer == NULL || buffer_size == 0 || (store != NULL && store->image_size_bytes != image_size_bytes))
    {
    ...
    uint16_t buffer_size)
{
    ...
    while (offset < image_size_bytes)
    {
        ...
        for (uint16_t index = 0; index < chunk; ++index)
        {
            uint8_t value;
            if (
                store != NULL && store->image_size_bytes != 0)
            {
