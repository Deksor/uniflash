...
static bool erase_bulk(struct uf_flash_service *service, uf_rom_offset_t ignored_address) {
    uint16_t timeout = UINT16_C(20000);
    uint8_t status;

    (void)ignored_address;
    if (service == NULL || service->chip == NULL || service->access.delay_us == NULL ||
        !uf_flash_service_command(service, UINT8_C(0x80)) || !uf_flash_service_command(service, UINT8_C(0x10))) {
        if (service != NULL) {
...
static bool program_byte(struct uf_flash_service *service, uf_rom_offset_t position, uf_phys_addr_t source_address) {
    if (service == NULL || service->chip == NULL || service->access.delay_us == NULL ||
    ...
    for (uint16_t offset = 0; offset < service->chip->page_size_bytes; ++offset) {
        ...
        do {
            if (!service->access.delay_us(service->access.context, UINT32_C(10)) ||
                !uf_flash_service_read_byte(service, position + offset, &status)) {
                service->error = UF_FLASH_ERROR_PROGRAM;
                return false;
            }
            --timeout;
...
static bool write_page(struct uf_flash_service *service, uf_rom_offset_t position, uf_phys_addr_t source_address) {
    for (uint16_t offset = 0; offset < service->chip->page_size_bytes; ++offset) {
        uint8_t value;
...
static bool program_page(struct uf_flash_service *service, uf_rom_offset_t position, uf_phys_addr_t source_address) {
    uint8_t expected;
    uint8_t status = 0;

    if (service == NULL || service->chip == NULL || service->access.delay_us == NULL ||
        service->access.read_source_byte == NULL || !source_byte(service, source_address + UINT32_C(127), &expected)) {
        if (service != NULL) {
            service->error = UF_FLASH_ERROR_PROGRAM;
        }
        return false;
    ...
    for (uint8_t attempt = 0; attempt < 4; ++attempt) {
        ...
        if (!uf_flash_service_command(service, UINT8_C(0xA0)) || !write_page(service, position, source_address) ||
...
static uint8_t
fwh_addresses(const struct uf_flash_service *service, uf_rom_offset_t address, uint8_t variant, uf_phys_addr_t *locks) {
    if (variant == 1) {
...
static bool protected_erase(struct uf_flash_service *service, uf_rom_offset_t address, uint8_t variant) {
    uf_phys_addr_t locks[2];
    uint8_t count;
    uf_flash_inner_erase_fn operation = variant == 1 ? uf_flash_erase_intel_sector : uf_flash_erase_intel_sector_u;
    ...
    return uf_flash_run_protected_erase(service, address, locks, count, UINT8_C(0xFC), operation);
}
...
#define DEFINE_ATMEL_WRAPPERS(suffix, variant_value)                                                                   \
    static bool program_fwh##suffix(struct uf_flash_service *service,                                                  \
        uf_rom_offset_t position,                                                                                      \
        uf_phys_addr_t source_address) {                                                                               \
        return protected_program(service, position, source_address, variant_value);                                    \
    }                                                                                                                  \
...
bool uf_flash_register_atmel_algorithms(uf_flash_algorithm_registry_t *registry) {
