...
)
{
    ...
    for (attempt = 0; attempt < 4; ++attempt) {
        ...
        if (
            !uf_flash_service_command(service, UINT8_C(0xA0))
            || !write_page(service, position, source_address)
            || !service->access.delay_us(
                service->access.context,
                UINT32_C(10000)
            )
...
)
{
    ...
    if (variant == 3) {
        if (address >= UINT32_C(0x70000)) {
            ...
            locks[0] = uf_flash_fwh_64k_lock_address(service, address);
            locks[1] = (
                address & UINT32_C(0xFFFF0000)
            ) + UINT32_C(0xFF800002) - service->chip->capacity_bytes;
        }
        return UINT8_C(2);
    }
...
uf_bool_t uf_flash_register_atmel_algorithms(
