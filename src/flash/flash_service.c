...
bool uf_flash_access_is_valid(const uf_flash_access_t *access) {
    if (access == NULL || access->read_byte == NULL || access->write_byte == NULL) {
        return false;
    }
    return true;
}
...
bool uf_flash_algorithm_registry_is_valid(const uf_flash_algorithm_registry_t *registry) {
    if (registry == NULL || registry->program[UF_FLASH_PROGRAM_NONE] != NULL ||
...
bool uf_flash_service_set_algorithms(uf_flash_service_t *service, const uf_flash_algorithm_registry_t *registry) {
    if (service == NULL || !uf_flash_algorithm_registry_is_valid(registry)) {
        return false;
    }
...
bool uf_flash_service_read_byte(uf_flash_service_t *service, uf_rom_offset_t address, uint8_t *value) {
    if (service == NULL || value == NULL || service->access.read_byte == NULL ||
...
bool uf_flash_service_write_byte(uf_flash_service_t *service, uf_rom_offset_t address, uint8_t value) {
    if (service == NULL || service->access.write_byte == NULL ||
        !uf_flash_service_range_is_valid(service, address, 1)) {
        return false;
    }
...
bool uf_flash_service_read_block(uf_flash_service_t *service,
    ...
    void *destination,
    uf_rom_size_t size_bytes) {
    if (service == NULL || (destination == NULL && size_bytes != 0) || service->access.read_block == NULL ||
        !uf_flash_service_range_is_valid(service, flash_address, size_bytes)) {
        return false;
    }
    return service->access.read_block(service->access.context, flash_address, destination, size_bytes);
}
...
bool uf_flash_service_write_block(uf_flash_service_t *service,
    ...
    uf_rom_offset_t flash_address,
    uf_rom_size_t size_bytes) {
    if (service == NULL || (source == NULL && size_bytes != 0) || service->access.write_block == NULL ||
        !uf_flash_service_range_is_valid(service, flash_address, size_bytes)) {
        return false;
    }
    return service->access.write_block(service->access.context, source, flash_address, size_bytes);
}
...
bool uf_flash_service_compare_block(uf_flash_service_t *service,
    ...
    uf_rom_size_t size_bytes,
    bool *equal) {
    if (service == NULL || (source == NULL && size_bytes != 0) || equal == NULL ||
        service->access.compare_block == NULL || !uf_flash_service_range_is_valid(service, flash_address, size_bytes)) {
        return false;
    }
    return service->access.compare_block(service->access.context, source, flash_address, size_bytes, equal);
}
...
bool uf_flash_service_command(uf_flash_service_t *service, uint8_t command) {
    ...
    if (!uf_flash_service_write_byte(service,
    ...
    return uf_flash_service_write_byte(service, (uf_rom_offset_t)UF_FLASH_COMMAND_ADDRESS_1, command);
}
...
static bool send_id_command(uf_flash_service_t *service, uf_flash_id_method_t method) {
    if (method == UF_FLASH_ID_METHOD_LEGACY) {
        if (!uf_flash_service_command(service, UINT8_C(0x80)) || !uf_flash_service_command(service, UINT8_C(0x60))) {
            return false;
        }
    ...
    return service->access.delay_us(service->access.context, UINT32_C(50));
}

static bool reset_after_id(uf_flash_service_t *service, uint8_t manufacturer_id) {
    uint8_t reset_command = manufacturer_id == UINT8_C(0x89) ? UINT8_C(0xFF) : UINT8_C(0xF0);

    if (!uf_flash_service_write_byte(service, 0, reset_command)) {
        return false;
    }
    return service->access.delay_us(service->access.context, UINT32_C(1000));
}

bool uf_flash_service_probe_id(uf_flash_service_t *service, uf_flash_id_method_t method, uf_flash_id_result_t *result) {
    uint8_t extension;

    if (service == NULL || result == NULL || service->access.delay_us == NULL ||
        (method != UF_FLASH_ID_METHOD_LEGACY && method != UF_FLASH_ID_METHOD_STANDARD)) {
        return false;
    }
    ...
    if (!uf_flash_service_read_byte(service, 0, &result->initial_manufacturer_id) ||
        !uf_flash_service_read_byte(service, 1, &result->initial_device_id) || !send_id_command(service, method) ||
        !uf_flash_service_read_byte(service, 0, &result->manufacturer_id)) {
        return false;
    }
    ...
    if (result->manufacturer_id == UINT8_C(0xC2)) {
        if (!reset_after_id(service, result->manufacturer_id) || !send_id_command(service, method)) {
            return false;
        }
    }
    ...
    result->valid = ((result->manufacturer_id != result->initial_manufacturer_id ||
...
bool uf_flash_service_accept_id(uf_flash_service_t *service, const uf_flash_id_result_t *result) {
    ...
    if (!uf_flash_service_select_chip(service, result->manufacturer_id, result->device_id)) {
        return false;
    }

    service->rom_base = UINT32_C(0) - (uf_phys_addr_t)service->chip->capacity_bytes;
    return true;
}
...
bool uf_flash_service_detect_current_window(uf_flash_service_t *service) {
    ...
    if (!uf_flash_service_probe_id(service, UF_FLASH_ID_METHOD_LEGACY, &result)) {
        return false;
    }
    ...
    if (!uf_flash_service_probe_id(service, UF_FLASH_ID_METHOD_STANDARD, &result)) {
        return false;
    }
    return uf_flash_service_accept_id(service, &result);
}

static bool select_rom_window(uf_flash_service_t *service, uf_phys_addr_t rom_base) {
    service->rom_base = rom_base;
    return service->access.select_window(service->access.context, rom_base);
}
...
bool uf_flash_service_detect(uf_flash_service_t *service) {
    ...
    for (uf_flash_id_method_t method = UF_FLASH_ID_METHOD_LEGACY; method <= UF_FLASH_ID_METHOD_STANDARD; ++method) {
        for (uint8_t exponent = minimum_exponent; exponent <= maximum_exponent; ++exponent) {
            uf_phys_addr_t candidate_base = fixed_rom_base;

            if (exponent != 0) {
                candidate_base = UINT32_C(0) - (UINT32_C(1) << exponent);
            }
            if (!select_rom_window(service, candidate_base) || !uf_flash_service_probe_id(service, method, &result)) {
                return false;
            }
            if (uf_flash_service_accept_id(service, &result)) {
                if (!service->access.select_window(service->access.context, service->rom_base)) {
                    uf_flash_service_clear_selection(service);
                    return false;
                }
                return true;
...
bool uf_flash_service_program(uf_flash_service_t *service, uf_rom_offset_t position, uf_phys_addr_t source_address) {
    if (service == NULL || service->chip == NULL || service->algorithms == NULL ||
        !uf_flash_service_range_is_valid(service, position, (uf_rom_size_t)service->chip->page_size_bytes)) {
        if (service != NULL) {
