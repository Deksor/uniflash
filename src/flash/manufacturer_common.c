...
)
{
    ...
    if (
        operation_address >= UINT32_C(0x38000)
        && operation_address < UINT32_C(0x3C000)
    ) {
...
)
{
    if (
        service == NULL
        || service->access.update_phys_byte == NULL
    ) {
        return UF_FALSE;
    }
