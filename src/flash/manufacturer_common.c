    ...
    uf_rom_offset_t operation_address)
{
    ...
    if (
        operation_address >= UINT32_C(0x38000) && operation_address < UINT32_C(0x3C000))
    {
        adjusted -= UINT32_C(0x4000);
    }
    return (
               adjusted & UINT32_C(0xFFFF8000)) +
           UINT32_C(0xFFC00002) - service->chip->capacity_bytes;
}

bool uf_flash_update_phys_byte(
...
{
    if (
        service == NULL || service->access.update_phys_byte == NULL)
    {
        return false;
    }
    return service->access.update_phys_byte(
...
bool uf_flash_run_protected_program(
    struct uf_flash_service *service,
    ...
    uf_flash_inner_program_fn operation)
{
    ...
    if (
        service == NULL || service->chip == NULL || operation == NULL)
    {
        if (service != NULL)
        {
...
bool uf_flash_run_protected_erase(
    struct uf_flash_service *service,
