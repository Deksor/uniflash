...
#define UF_ROM_BEGIN(symbol, manufacturer_id, manufacturer_name) static const uf_flash_chip_t symbol##_chips[] = {
#define UF_ROM_CHIP(device_id,                                                                                         \
    chip_name,                                                                                                         \
    capacity_bytes,                                                                                                    \
    page_size_bytes,                                                                                                   \
    behavior,                                                                                                          \
...
#define UF_ROM_BEGIN(symbol, manufacturer_id, manufacturer_name)                                                       \
    {manufacturer_name, (uint8_t)(manufacturer_id), symbol##_chips, (uint16_t)UF_ARRAY_COUNT(symbol##_chips)},
#define UF_ROM_CHIP(device_id,                                                                                         \
    chip_name,                                                                                                         \
    capacity_bytes,                                                                                                    \
    page_size_bytes,                                                                                                   \
...
const uint16_t uf_rom_manufacturer_count = (uint16_t)UF_ARRAY_COUNT(uf_rom_manufacturers);

static bool behavior_is_valid(uf_flash_behavior_t behavior) {
    switch (behavior) {
...
static bool chip_geometry_is_valid(const uf_flash_chip_t *chip) {
    ...
    for (uint8_t index = 0; index < chip->sector_region_count; ++index) {
        ...
        if (region->sector_size_bytes > UINT32_MAX / (uf_rom_size_t)region->sector_count) {
            return false;
        }
        if (total_size > UINT32_MAX - region->sector_size_bytes * (uf_rom_size_t)region->sector_count) {
            return false;
        }
        total_size += region->sector_size_bytes * (uf_rom_size_t)region->sector_count;
    }

    for (uint8_t index = chip->sector_region_count; index < UF_FLASH_MAX_SECTOR_REGIONS; ++index) {
        const uf_flash_sector_region_t *region = &chip->sector_regions[index];
...
static bool chip_is_valid(const uf_flash_chip_t *chip) {
    if (chip->name == NULL || chip->name[0] == '\0' || chip->capacity_bytes == 0 || chip->page_size_bytes == 0 ||
    ...
    if (chip->program_algorithm == UF_FLASH_PROGRAM_NONE || chip->program_algorithm >= UF_FLASH_PROGRAM_COUNT ||
...
const uf_flash_manufacturer_t *uf_rom_find_manufacturer(uint8_t manufacturer_id) {
    for (uint16_t index = 0; index < uf_rom_manufacturer_count; ++index) {
        if (uf_rom_manufacturers[index].manufacturer_id == manufacturer_id) {
...
const uf_flash_chip_t *uf_rom_find_chip(const uf_flash_manufacturer_t *manufacturer, uint8_t device_id) {
...
bool uf_rom_database_validate(void) {
    for (uint16_t manufacturer_index = 0; manufacturer_index < uf_rom_manufacturer_count; ++manufacturer_index) {
        const uf_flash_manufacturer_t *manufacturer = &uf_rom_manufacturers[manufacturer_index];
        if (manufacturer->name == NULL || manufacturer->name[0] == '\0' || manufacturer->chips == NULL ||
            manufacturer->chip_count == 0) {
            return false;
        }

        for (uint16_t previous_index = 0; previous_index < manufacturer_index; ++previous_index) {
            if (uf_rom_manufacturers[previous_index].manufacturer_id == manufacturer->manufacturer_id) {
                return false;
            }
        }

        for (uint16_t chip_index = 0; chip_index < manufacturer->chip_count; ++chip_index) {
            if (!chip_is_valid(&manufacturer->chips[chip_index])) {
                return false;
            }
            for (uint16_t previous_index = 0; previous_index < chip_index; ++previous_index) {
                if (manufacturer->chips[previous_index].device_id == manufacturer->chips[chip_index].device_id) {
                    return false;
                }
            }
        }
