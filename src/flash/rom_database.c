...
static bool chip_geometry_is_valid(const uf_flash_chip_t *chip)
{
    ...
    for (index = 0; index < chip->sector_region_count; ++index)
    {
        ...
        if (
            region->sector_size_bytes > UINT32_MAX / (uf_rom_size_t)region->sector_count)
        {
            return false;
        }
        if (
            total_size > UINT32_MAX - region->sector_size_bytes * (uf_rom_size_t)region->sector_count)
        {
            return false;
        }
        total_size +=
...
{
    if (
        chip->name == NULL || chip->name[0] == '\0' || chip->capacity_bytes == 0 || chip->page_size_bytes == 0 || (uf_rom_size_t)chip->page_size_bytes > chip->capacity_bytes)
    {
        return false;
    }
    ...
    if (
        chip->program_algorithm == UF_FLASH_PROGRAM_NONE || chip->program_algorithm >= UF_FLASH_PROGRAM_COUNT || chip->erase_algorithm >= UF_FLASH_ERASE_COUNT)
    {
        return false;
    }
...
bool uf_rom_database_validate(void)
{
        ...
        ++manufacturer_index)
    {
        ...
        {
            if (
                uf_rom_manufacturers[previous_index].manufacturer_id == manufacturer->manufacturer_id)
            {
                return false;
            }
        }
             ...
             ++chip_index)
        {
            ...
            {
                if (
                    manufacturer->chips[previous_index].device_id == manufacturer->chips[chip_index].device_id)
                {
                    return false;
                }
            }
