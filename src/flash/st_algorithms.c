...
)
{
    ...
    if (variant == 2) {
        if (
            address
            >= service->chip->capacity_bytes - UINT32_C(0x8000)
        ) {
            locks[0] = UINT32_C(0xFFBF8002);
            if (
                address
                >= service->chip->capacity_bytes - UINT32_C(0x6000)
            ) {
                locks[0] = address
                    >= service->chip->capacity_bytes - UINT32_C(0x4000)
                    ? UINT32_C(0xFFBFC002)
                    : UINT32_C(0xFFBFA002);
            }
        } else {
    ...
    if (
        (variant == 3 && (
            address < UINT32_C(0x10000)
            || address
                >= service->chip->capacity_bytes - UINT32_C(0x20000)
        ))
        || (variant == 4 && (
            address < UINT32_C(0x20000)
            || address
...
static uf_bool_t program_##suffix( \
    struct uf_flash_service *service, \
    uf_rom_offset_t position, \
    uf_phys_addr_t source_address \
) \
...
static uf_bool_t erase_##suffix( \
    struct uf_flash_service *service, \
    uf_rom_offset_t address \
) \
{ \
