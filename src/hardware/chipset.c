...
typedef struct chipset_id {
    ...
    uf_bool_t detect_lpc;
} chipset_id_t;

#define CHIP(vendor_value, device_value, text, method_value) \
    { vendor_value, device_value, text, method_value, UF_FALSE }
#define CHIP_LPC(vendor_value, device_value, text, method_value) \
    { vendor_value, device_value, text, method_value, UF_TRUE }
...
static uf_bool_t indexed_read8(
...
)
{
    return (
        ...
        && hardware->in8(hardware->context, data_port, value)
    ) ? UF_TRUE : UF_FALSE;
}

static uf_bool_t indexed_write8(
    const uf_hardware_t *hardware,
    uf_io_port_t index_port,
    uf_io_port_t data_port,
    uint8_t index,
...
)
{
    return (
        hardware->out8(hardware->context, index_port, index)
        && hardware->out8(hardware->context, data_port, value)
    ) ? UF_TRUE : UF_FALSE;
}

static uf_bool_t set_north_enabled(
...
static uf_bool_t save_and_write(
    uf_chipset_t *chipset,
    uint8_t slot,
    uint8_t reg,
    uint32_t and_mask,
...
static uf_bool_t set_south_simple(
    uf_chipset_t *chipset,
    uf_bool_t enabled,
    uint8_t reg,
    uint32_t and_mask,
...
static uf_bool_t set_sis_enabled(
    uf_chipset_t *chipset,
    uf_bool_t enabled
)
{
    ...
    if (subtype == 1 || subtype == 2) {
        ...
        if (
            (chipset->south_saved_valid & UINT8_C(0x02)) != 0
            && !indexed_write8(
            hardware, UINT16_C(0x22), UINT16_C(0x23),
        ...
        if (
            subtype == 1
            && (chipset->south_saved_valid & UINT8_C(0x04)) != 0
            && !indexed_write8(
                hardware, UINT16_C(0x22), UINT16_C(0x23),
                UINT8_C(0x70), (uint8_t)chipset->south_saved[2]
            )
        ) {
            return UF_FALSE;
        }
...
static uf_bool_t set_south_enabled(
    uf_chipset_t *chipset,
    uf_bool_t enabled
)
{
    ...
    case 2:
        ...
        return restore_reg(chipset, 0, UINT8_C(0x4C))
            && restore_reg(chipset, 1, UINT8_C(0xE0));
    case 3:
        return set_south_simple(
            chipset,
            enabled,
    ...
    case 4:
        ...
        if (
            !restore_reg(chipset, 0, UINT8_C(0x44))
            || !restore_reg(chipset, 1, UINT8_C(0x40))
        ) {
            return UF_FALSE;
        }
        return (subtype & 1) == 0 || (
            restore_reg(chipset, 2, UINT8_C(0x78))
            && restore_reg(chipset, 3, UINT8_C(0x7C))
        );
    case 5:
    ...
    case 9: {
        ...
        if (
            !restore_reg(chipset, 0, UINT8_C(0x40))
            || !restore_reg(chipset, 1, UINT8_C(0x70))
        ) {
            return UF_FALSE;
        }
        if (
    ...
    case 12:
        ...
        return restore_reg(chipset, 0, UINT8_C(0x44))
            && restore_reg(chipset, 1, UINT8_C(0x48));
    case 13:
        return set_south_simple(
    ...
    case 16:
        ...
        return restore_reg(chipset, 0, UINT8_C(0x44))
            && restore_reg(chipset, 1, UINT8_C(0x70))
            && restore_reg(chipset, 2, UINT8_C(0x74))
            && restore_reg(chipset, 3, UINT8_C(0x78))
    ...
    case 17:
        ...
        return restore_reg(chipset, 0, UINT8_C(0xDC))
            && restore_reg(chipset, 1, UINT8_C(0xD8));
    default:
        return UF_FALSE;
    }
}

static uf_bool_t lpc_unlock(uf_chipset_t *chipset)
{
    const uf_hardware_t *hardware = chipset->pci.hardware;
    uint8_t sequence[] = {
...
static uf_bool_t set_lpc_enabled(
    uf_chipset_t *chipset,
    uf_bool_t enabled
)
{
...
)
{
    ...
    for (index = 0; index < count; ++index) {
        if (
            ids[index].vendor == vendor
            && ids[index].device == device
        ) {
            return &ids[index];
        }
    }
...
static uf_bool_t visit_chipset(
    void *context,
    const uf_pci_function_info_t *device
)
{
    ...
    if (
        id != NULL
        && !detect->chipset->south_found
        && device->class_code == UINT8_C(0x06)
        && (
            device->subclass == UINT8_C(0x01)
            || device->subclass == UINT8_C(0x80)
            || device->subclass == UINT8_C(0x00)
        )
    ) {
        detect->chipset->south = *device;
        detect->chipset->south_found = UF_TRUE;
...
static uf_bool_t detect_lpc_at(
    uf_chipset_t *chipset,
    uf_io_port_t base
)
{
    ...
    uint8_t low;
    uint16_t id;
    uf_bool_t ite = UF_FALSE;

    chipset->lpc_base = base;
    if (
        !lpc_unlock(chipset)
        || !lpc_read(chipset, UINT8_C(0x20), &high)
        || !lpc_read(chipset, UINT8_C(0x21), &low)
    ) {
        chipset->lpc_base = 0;
        return UF_FALSE;
    }
...
)
{
    ...
    if (detect.wants_lpc) {
        if (
            !detect_lpc_at(chipset, UINT16_C(0x2E))
            && !detect_lpc_at(chipset, UINT16_C(0x4E))
        ) {
            chipset->lpc_base = 0;
            chipset->lpc_method = 0;
        }
...
)
{
    ...
    if (
        chipset == NULL
        || chipset->pci.hardware == NULL
        || chipset->rom_enabled == enabled
    ) {
        return chipset != NULL ? UF_TRUE : UF_FALSE;
    }
    if (enabled) {
        chipset->south_saved_valid = 0;
        north_ok = set_north_enabled(chipset, UF_TRUE);
        south_ok = north_ok
            ? set_south_enabled(chipset, UF_TRUE)
            : UF_FALSE;
        lpc_ok = south_ok
            ? set_lpc_enabled(chipset, UF_TRUE)
            : UF_FALSE;
        if (!lpc_ok) {
            if (north_ok) {
                (void)set_south_enabled(chipset, UF_FALSE);
            }
            if (north_ok) {
                (void)set_north_enabled(chipset, UF_FALSE);
            }
            return UF_FALSE;
        }
    } else {
        lpc_ok = set_lpc_enabled(chipset, UF_FALSE);
        south_ok = set_south_enabled(chipset, UF_FALSE);
        north_ok = set_north_enabled(chipset, UF_FALSE);
