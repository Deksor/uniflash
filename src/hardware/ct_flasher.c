...
static bool port_is_ct_flasher(
    ...
    uint8_t *status)
{
    ...
    if (
        !hardware->in8(hardware->context, port, &value) || (value & UINT8_C(0xC7)) != UINT8_C(0x80) || !hardware->out8(hardware->context, port, UINT8_C(0)) || !hardware->in8(hardware->context, port, &value) || (value & UINT8_C(0xC7)) != UINT8_C(0x80))
    {
        return false;
    }
    *status = value;
