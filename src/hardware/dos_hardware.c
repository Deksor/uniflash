...
#pragma aux dos_phys_write8_raw = "push es"                                                                            \
                                  "mov cx,ax"                                                                          \
                                  "movzx esi,dx"                                                                       \
                                  "shl esi,16"                                                                         \
                                  "mov si,cx"                                                                          \
...
static bool dos_in8(void *context, uf_io_port_t port, uint8_t *value) {
    (void)context;
    if (value == NULL) {
        return false;
    }
...
static bool dos_in16(void *context, uf_io_port_t port, uint16_t *value) {
    (void)context;
    if (value == NULL) {
        return false;
    }
...
static bool dos_in32(void *context, uf_io_port_t port, uint32_t *value) {
    (void)context;
    if (value == NULL) {
        return false;
    }
...
static bool dos_out8(void *context, uf_io_port_t port, uint8_t value) {
    (void)context;
    dos_out8_raw(port, value);
    return true;
}

static bool dos_out16(void *context, uf_io_port_t port, uint16_t value) {
    (void)context;
    dos_out16_raw(port, value);
    return true;
}

static bool dos_out32(void *context, uf_io_port_t port, uint32_t value) {
    (void)context;
    dos_out32_raw(port, value);
    return true;
}

static bool dos_phys_read8(void *context, uf_phys_addr_t address, uint8_t *value) {
    (void)context;
    if (value == NULL) {
        return false;
    }
...
static bool dos_phys_write8(void *context, uf_phys_addr_t address, uint8_t value) {
    (void)context;
    dos_phys_write8_raw(address, value);
    return true;
}

static bool dos_delay_us(void *context, uint32_t microseconds) {
     ...
     */
    while (remaining > 0) {
        uint32_t chunk_us = remaining > UINT32_C(50000) ? UINT32_C(50000) : remaining;
...
bool uf_dos_hardware_init(uf_hardware_t *hardware) {
    ...
    return (dos_flat_real_on() && uf_dos_hardware_init_io(hardware));
}
