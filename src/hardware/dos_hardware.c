...
static uf_bool_t dos_flat_real_on(void)
{
    uf_bool_t result;
...
#pragma aux dos_in32_raw = \
    ...
    "mov edx,eax" \
    "shr edx,16" \
    parm [dx] value [dx ax] modify exact [ax dx];
...
#pragma aux dos_phys_read8_raw = \
    ...
    "mov al,es:[esi]" \
    "pop es" \
    parm [dx ax] value [al] modify exact [ax bx si];
...
#pragma aux dos_phys_write8_raw = \
    ...
    "mov es:[esi],bl" \
    "pop es" \
    parm [dx ax] [bl] modify exact [cx si];
...
static uf_bool_t dos_phys_read8(
    void *context,
...
static uf_bool_t dos_phys_write8(
    void *context,
    uf_phys_addr_t address,
    uint8_t value
)
...
)
{
    ...
    while (remaining > 0) {
        ...
        uint16_t target = (uint16_t)(
            (chunk_us * UINT32_C(1193) + UINT32_C(999)) / UINT32_C(1000)
        );
        uint16_t start;
        uint16_t current;
        uint16_t elapsed;
...
static uf_bool_t dos_interrupts_disable(void *context)
{
    (void)context;
    dos_cli_raw();
    return UF_TRUE;
...
static uf_bool_t dos_interrupts_restore(void *context)
{
    (void)context;
    dos_sti_raw();
    return UF_TRUE;
...
uf_bool_t uf_dos_hardware_init_io(uf_hardware_t *hardware)
{
    if (hardware == NULL) {
        return UF_FALSE;
    }
...
uf_bool_t uf_dos_hardware_init(uf_hardware_t *hardware)
{
    if (hardware == NULL) {
        return UF_FALSE;
    }
    return (
        dos_flat_real_on()
        && uf_dos_hardware_init_io(hardware)
    ) ? UF_TRUE : UF_FALSE;
}
