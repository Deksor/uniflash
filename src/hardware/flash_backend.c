...
static bool
map_address(uf_system_flash_backend_t *backend, uf_rom_offset_t offset, bool writing, uf_phys_addr_t *physical) {
    uf_phys_addr_t base = writing ? backend->write_base : backend->rom_base;
    ...
    if (backend->bank_size != 0) {
        ...
        bank = (uint8_t)bank32;
        if ((!backend->bank_selected || bank != backend->current_bank) &&
...
static bool shadow(uf_system_flash_backend_t *backend, bool disable) {
    if (backend->control_shadow == NULL) {
        return true;
    }
    return backend->control_shadow(backend->shadow_context, disable);
}

static bool read_byte(void *context, uf_rom_offset_t address, uint8_t *value) {
    uf_system_flash_backend_t *backend = context;
    uf_phys_addr_t physical;
    bool result;

    if (value == NULL || !map_address(backend, address, false, &physical) || !shadow(backend, true)) {
        return false;
    }
    result = backend->hardware->phys_read8(backend->hardware->context, physical, value);
...
static bool write_byte(void *context, uf_rom_offset_t address, uint8_t value) {
    uf_system_flash_backend_t *backend = context;
    uf_phys_addr_t physical;
    ...
    return backend->hardware->phys_write8(backend->hardware->context, physical, value);
}

static bool read_block(void *context, uf_rom_offset_t flash_address, void *destination, uf_rom_size_t size_bytes) {
    uf_system_flash_backend_t *backend = context;
...
static bool write_block(void *context, const void *source, uf_rom_offset_t flash_address, uf_rom_size_t size_bytes) {
    uf_system_flash_backend_t *backend = context;
...
static bool
compare_block(void *context, const void *source, uf_rom_offset_t flash_address, uf_rom_size_t size_bytes, bool *equal) {
    uf_system_flash_backend_t *backend = context;
    const uint8_t *bytes = source;
    if (equal == NULL) {
        return false;
...
static bool read_source_byte(void *context, uf_phys_addr_t source_address, uint8_t *value) {
    uf_system_flash_backend_t *backend = context;

    return backend->hardware->phys_read8(backend->hardware->context, source_address, value);
}

static bool update_phys_byte(void *context, uf_phys_addr_t address, uint8_t and_mask, uint8_t or_mask) {
    uf_system_flash_backend_t *backend = context;
    uint8_t value;

    if (!backend->hardware->phys_read8(backend->hardware->context, address, &value)) {
        return false;
    }
    value = (value & and_mask) | or_mask;
    return backend->hardware->phys_write8(backend->hardware->context, address, value);
}

static bool set_write_lock(void *context, uf_phys_addr_t address, bool locked) {
    return update_phys_byte(context,
        address,
...
bool uf_system_flash_backend_set_banking(uf_system_flash_backend_t *backend,
    ...
    void *bank_context) {
    if (backend == NULL || bank_size == 0 || select_bank == NULL) {
        return false;
    }
