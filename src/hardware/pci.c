...
)
{
    return UINT32_C(0x80000000)
        ...
        | (((uint32_t)address.device & UINT32_C(0x1F)) << 11)
        | (((uint32_t)address.function & UINT32_C(0x07)) << 8)
        | ((uint32_t)reg & UINT32_C(0xFC));
}
...
)
{
    ...
    *value = (uint16_t)(
        data >> (((uint16_t)reg & UINT16_C(2)) * UINT16_C(8))
    );
    return UF_TRUE;
}
...
)
{
    ...
    *value = (uint8_t)(
        data >> (((uint16_t)reg & UINT16_C(3)) * UINT16_C(8))
    );
    return UF_TRUE;
}
...
)
{
    ...
    if (
        device == NULL
        || !uf_pci_read32(pci, address, UINT8_C(0), &identity)
...
)
{
    ...
    for (bus_number = 0; bus_number <= UINT8_MAX; ++bus_number) {
        ...
        for (device_number = 0; device_number < 32; ++device_number) {
            ...
            function_count = (
                function_zero.header_type & UINT8_C(0x80)
            ) != 0 ? UINT8_C(8) : UINT8_C(1);
            for (
                function_number = 1;
                ...
                if (
                    uf_pci_probe(pci, address, &device)
                    && !visit(context, &device)
                ) {
                    return UF_TRUE;
                }
            }
