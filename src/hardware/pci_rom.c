    ...
    uf_rom_size_t *size)
{
    ...
    if (
        pci->hardware->interrupts_disable != NULL && pci->hardware->interrupts_restore != NULL)
    {
    ...
    const uf_pci_function_info_t *device)
{
    ...
    if (
        scan->list->count < UF_PCI_ROM_MAX_DEVICES && probe_rom_size(scan->pci, device, &size))
    {
    ...
    const uf_hardware_t *hardware)
{
    ...
    if (
        list == NULL || !uf_pci_bus_init(&pci, hardware))
    {
        return false;
    }
    ...
    const uf_pci_rom_device_t *device)
{
    ...
    needs_io = backend->kind != UF_PCI_FLASH_MEMORY || (device->pci_device.vendor_id == 0x10B8 && device->pci_device.device_id == 0x0006);
    if (
        needs_io && !find_io_base(
                        &backend->pci,
    ...
    if (
        backend->kind == UF_PCI_FLASH_MEMORY && device->pci_device.vendor_id == 0x9004 && device->pci_device.device_id == 0x6915 && !find_memory_base(&backend->pci, device->pci_device.address, &backend->memory_base))
    {
        return false;
    }
    return true;
    ...
    uint8_t *value)
{
    ...
    {
        uf_io_port_t port = backend->io_base + (backend->kind == UF_PCI_FLASH_REALTEK
                                                    ? UINT16_C(0xD4)
                                                    : UINT16_C(0x30));
        if (
            !hardware->out32(
    ...
    uint8_t value)
{
    ...
    case UF_PCI_FLASH_REALTEK_1000:
    {
        ...
        return hardware->out32(
            hardware->context, port,
            address | UINT32_C(0x160000) | ((uint32_t)value << 24));
    }
    case UF_PCI_FLASH_VIA:
        if (
            !hardware->out16(
...
static bool access_write_block(
...
static bool access_update_phys(
...
static bool access_lock(
    void *context, uf_phys_addr_t address, bool locked)
{
    ...
    bool enabled)
{
    ...
    if (enabled)
    {
        ...
        if (
            backend->device.pci_device.vendor_id != 0x9004 || backend->device.pci_device.device_id != 0x6915)
        {
            backend->memory_base = UINT32_C(0x80000000);
        }
        if (
            backend->device.pci_device.vendor_id == 0x10B8 && backend->device.pci_device.device_id == 0x0006)
        {
            if (!backend->hardware->in32(
                    backend->hardware->context,
        ...
        if (
            backend->device.pci_device.vendor_id == 0x121A && backend->device.pci_device.device_id == 0x0003)
        {
            if (!uf_pci_read32(
    ...
    {
        if (
            backend->device.pci_device.vendor_id == 0x10B8 && backend->device.pci_device.device_id == 0x0006 && !backend->hardware->out32(backend->hardware->context, backend->io_base + UINT16_C(0x10), backend->saved_card_register))
        {
            return false;
        }
        if (
            backend->device.pci_device.vendor_id == 0x121A && backend->device.pci_device.device_id == 0x0003 && !uf_pci_write32(&backend->pci, address, UINT8_C(0x14), backend->saved_card_register))
        {
            return false;
        }
        if (
