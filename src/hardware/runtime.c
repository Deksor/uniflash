    ...
    bool disable)
{
    ...
    return uf_pci_write32(
        ...
        UINT8_C(0x4C),
        runtime->chipset.north_saved[0] & (disable
                                               ? UINT32_C(0x00FF00FF)
                                               : UINT32_C(0x00FFFFFF)));
}
    ...
    uf_phys_addr_t initial_rom_base)
{
    ...
    if (
        !uf_dos_hardware_init(&runtime->hardware) || !uf_chipset_detect(&runtime->chipset, &runtime->hardware))
    {
        memset(runtime, 0, sizeof(*runtime));
        return false;
    }
    ...
    if (
        !uf_system_flash_backend_init(
            ...
            &runtime->hardware,
            rom_base) ||
        (runtime->chipset.north_method == UINT16_C(0x0300) && !uf_system_flash_backend_set_shadow_control(
                                                                  &runtime->flash_backend,
                                                                  aladdin_shadow_control,
                                                                  runtime)) ||
        (runtime->chipset.north_method == UINT16_C(0x0301) && !uf_system_flash_backend_set_banking(
                                                                  &runtime->flash_backend,
                                                                  UINT32_C(0x10000),
                                                                  UINT32_C(0x000F0000),
                                                                  finali_select_bank,
        ...
        !uf_flash_service_init(
            ...
            &runtime->flash_access,
            rom_base) ||
        !uf_flash_register_all_algorithms(&runtime->algorithms) || !uf_flash_service_set_algorithms(&runtime->flash, &runtime->algorithms))
    {
        memset(runtime, 0, sizeof(*runtime));
        return false;
    }
...
{
    if (
        runtime == NULL || !runtime->initialized)
    {
        return false;
    }
    if (runtime->flash_target == UF_RUNTIME_FLASH_PCI)
    {
        ...
        runtime->flash_target == UF_RUNTIME_FLASH_SYSTEM && !uf_chipset_rom_set_enabled(&runtime->chipset, enabled))
    {
        return false;
    }
...
bool uf_runtime_use_system_rom(uf_runtime_t *runtime)
{
    ...
    if (
        runtime == NULL || !runtime->initialized || runtime->rom_enabled || !uf_system_flash_backend_make_access(&runtime->flash_backend, &access) || !set_flash_access(runtime, &access, runtime->flash_backend.rom_base))
    {
        return false;
    }
    runtime->flash_target = UF_RUNTIME_FLASH_SYSTEM;
    ...
    bool size_512k)
{
    ...
    if (
        runtime == NULL || !runtime->initialized || runtime->rom_enabled || !uf_ct_flasher_detect(&runtime->ct_flasher, &runtime->hardware, requested_port, size_512k) || !uf_ct_flasher_make_access(&runtime->ct_flasher, &access) || !set_flash_access(runtime, &access, runtime->ct_flasher.window_base))
    {
        return false;
    }
    ...
    const uf_pci_rom_device_t *device)
{
    ...
    if (
        runtime == NULL || !runtime->initialized || runtime->rom_enabled || !uf_pci_rom_backend_init(&runtime->pci_rom, &runtime->hardware, device) || !uf_pci_rom_backend_make_access(&runtime->pci_rom, &access) || !set_flash_access(runtime, &access, runtime->pci_rom.memory_base))
    {
        return false;
    }
