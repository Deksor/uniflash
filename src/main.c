...
typedef struct uf_cli_options {
    ...
    uf_bool_t pci_address_set;
    uf_bool_t quit_after_action;
    uf_bool_t monochrome;
    uf_bool_t force_id;
    uint16_t forced_id;
} uf_cli_options_t;
...
static uf_bool_t option_is(const char *argument, const char *name)
{
    if (
        argument == NULL
        || (*argument != '-' && *argument != '/')
    ...
    return *argument == '\0' && *name == '\0' ? UF_TRUE : UF_FALSE;
}
...
static uf_bool_t argument_is_option(const char *argument)
{
    return (
        argument != NULL
...
static uf_bool_t parse_decimal_byte(const char *text, uint8_t *value)
{
    char *end;
...
static uf_bool_t parse_hex_address(
    const char *text,
...
)
{
    ...
    *address = (uf_phys_addr_t)value;
    return (unsigned long)*address == value ? UF_TRUE : UF_FALSE;
}

static uf_bool_t parse_options(
    int argc,
    char **argv,
...
{
    int index;
    uf_bool_t action_seen = UF_FALSE;
    ...
    for (index = 1; index < argc; ++index) {
        if (option_is(argv[index], "H") || option_is(argv[index], "?")) {
            ...
            option_is(argv[index], "DUMP")
            || option_is(argv[index], "SAVE")
            || option_is(argv[index], "BOOTBLOCK")
            || option_is(argv[index], "COMPARE")
        ) {
            ...
            options->quit_after_action = UF_TRUE;
        } else if (option_is(argv[index], "FORCE")) {
            uf_phys_addr_t forced;
            if (
                options->force_id
                || index + 1 >= argc
                || strlen(argv[index + 1]) != 4
                || !parse_hex_address(argv[++index], &forced)
                || forced > UINT16_MAX
            ) {
                return UF_FALSE;
            }
            options->forced_id = (uint16_t)forced;
            options->force_id = UF_TRUE;
            ...
            if (
                index + 1 < argc
                && !argument_is_option(argv[index + 1])
            ) {
                if (
                    !parse_hex_address(argv[++index], &port)
                    || port > UINT16_MAX
                ) {
                    return UF_FALSE;
                }
                ...
                if (
                    options->ct_port < UINT16_C(0x200)
                    || options->ct_port > UINT16_C(0x3FC)
                ) {
                    options->ct_port = 0;
                }
            }
            ...
            if (
                ...
                if (
                    index + 3 >= argc
                    || !parse_decimal_byte(argv[index + 1], &bus)
                    || !parse_decimal_byte(argv[index + 2], &device)
                    || !parse_decimal_byte(argv[index + 3], &function)
                    || device > 31
                    || function > 7
                ) {
                    return UF_FALSE;
                ...
                options->pci_address_set = UF_TRUE;
                index += 3;
            }
        } else if (option_is(argv[index], "BASE")) {
            if (
                index + 1 >= argc
                || !parse_hex_address(argv[++index], &options->rom_base)
            ) {
                return UF_FALSE;
            }
        } else {
...
static uf_bool_t select_flash_target(
...
)
{
    ...
    if (options->target == UF_CLI_TARGET_SYSTEM) {
        return uf_runtime_set_rom_enabled(runtime, UF_TRUE);
    }
    if (options->target == UF_CLI_TARGET_CT) {
        return uf_runtime_use_ct_flasher(
            runtime, options->ct_port, UF_FALSE
        );
    }
    ...
    if (!options->pci_address_set) {
        ...
        return (
            ...
            && uf_runtime_set_rom_enabled(runtime, UF_TRUE)
        ) ? UF_TRUE : UF_FALSE;
    }
    for (index = 0; index < list.count; ++index) {
        ...
        if (
            address->bus == options->pci_address.bus
            && address->device == options->pci_address.device
            && address->function == options->pci_address.function
        ) {
            return (
                uf_runtime_use_pci_rom(runtime, &list.devices[index])
                && uf_runtime_set_rom_enabled(runtime, UF_TRUE)
            ) ? UF_TRUE : UF_FALSE;
        }
    }
...
static uf_bool_t select_forced_chip(
    uf_runtime_t *runtime,
    const uf_cli_options_t *options
)
{
    ...
    if (options->target == UF_CLI_TARGET_SYSTEM) {
        ...
        if (base == 0) {
            base = UINT32_C(0)
                - runtime->flash.chip->capacity_bytes;
        }
...
static int run_read_only(const uf_cli_options_t *options)
{
    ...
    uf_bool_t runtime_ready = UF_FALSE;
    uf_bool_t rom_enabled = UF_FALSE;
    uf_bool_t xms_ready;
    int exit_code = 1;
    ...
    if (options->action == UF_CLI_CMOS_SAVE) {
        ...
        uint8_t last_index;
        if (
            !uf_dos_hardware_init_io(&cmos_hardware)
            || !uf_cmos_detect_last_index(
                &cmos_hardware, &last_index
            )
            || !uf_cmos_save_file(
                &cmos_hardware, last_index, options->path
            )
        ) {
    ...
    if (
        options->target == UF_CLI_TARGET_CT
        && runtime.flash.chip->capacity_bytes >= UF_KIB(512)
    ) {
        runtime.ct_flasher.size_512k = UF_TRUE;
    }
    if (
        (
            options->target == UF_CLI_TARGET_CT
        ...
        || (
            options->target == UF_CLI_TARGET_PCI
            && runtime.flash.chip->capacity_bytes
                > runtime.pci_rom.device.maximum_size
        )
    ) {
