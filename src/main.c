...
static bool parse_options(int argc, char **argv, uf_cli_options_t *options) {
    int index;
    ...
    for (index = 1; index < argc; ++index) {
        if (option_is(argv[index], "H") || option_is(argv[index], "?")) {
            ...
            action_seen = true;
        } else if (option_is(argv[index], "DUMP") || option_is(argv[index], "SAVE") ||
                   option_is(argv[index], "BOOTBLOCK") || option_is(argv[index], "COMPARE")) {
            ...
            if (options->force_id || index + 1 >= argc || strlen(argv[index + 1]) != 4 ||
                !parse_hex_address(argv[++index], &forced) || forced > UINT16_MAX) {
                return false;
            }
            ...
            if (index + 1 < argc && !argument_is_option(argv[index + 1])) {
                if (!parse_hex_address(argv[++index], &port) || port > UINT16_MAX) {
                    return false;
                }
            ...
            if (index + 1 < argc && !argument_is_option(argv[index + 1])) {
                if (index + 3 >= argc || !parse_decimal_byte(argv[index + 1], &bus) ||
                    !parse_decimal_byte(argv[index + 2], &device) || !parse_decimal_byte(argv[index + 3], &function) ||
            ...
            if (index + 1 >= argc || !parse_hex_address(argv[++index], &options->rom_base)) {
                return false;
            }
        } else {
            return false;
...
static bool select_flash_target(uf_runtime_t *runtime, const uf_cli_options_t *options) {
    ...
    for (uint8_t index = 0; index < list.count; ++index) {
        const uf_pci_address_t *address = &list.devices[index].pci_device.address;
        if (address->bus == options->pci_address.bus && address->device == options->pci_address.device &&
            ...
            return (uf_runtime_use_pci_rom(runtime, &list.devices[index]) && uf_runtime_set_rom_enabled(runtime, true));
        }
    }
    return false;
}
