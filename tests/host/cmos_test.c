...
static uf_bool_t in8(void *context, uf_io_port_t port, uint8_t *value)
{
    ...
    if (port == 0x71) {
        *value = mock->standard[
            mock->standard_index % mock->standard_size
        ];
        return UF_TRUE;
    }
    if (port == 0x73) {
...
static uf_bool_t delay_us(void *context, uint32_t value)
{
    (void)context;
    (void)value;
    return UF_TRUE;
...
int main(void)
{
    ...
    init_hardware(&hardware, &mock, 64, UF_FALSE);
    ...
    init_hardware(&hardware, &mock, 128, UF_FALSE);
    assert(uf_cmos_detect_last_index(&hardware, &last));
    assert(last == 0x7F);
    assert(uf_cmos_read(
    ...
    init_hardware(&hardware, &mock, 128, UF_TRUE);
    saved_10 = mock.standard[0x10];
