...
static bool in8(void *context, uf_io_port_t port, uint8_t *value) {
    ...
    if (port == 0x73) {
        *value = mock->extended_present ? mock->extended[mock->extended_index] : 0xFF;
        return true;
    }
    *value = 0;
    return true;
...
static bool out16(void *context, uf_io_port_t port, uint16_t value) { return out8(context, port, (uint8_t)value); }

static bool out32(void *context, uf_io_port_t port, uint32_t value) { return out8(context, port, (uint8_t)value); }
...
static void init_hardware(uf_hardware_t *hardware, cmos_mock_t *mock, uint8_t standard_size, bool extended_present) {
    uint16_t index;
...
int main(void) {
    ...
    assert(uf_cmos_detect_last_index(&hardware, &last));
    assert(last == 0x7F);
    assert(uf_cmos_read(&hardware, last, data, sizeof(data), &size));
    assert(size == 114);
