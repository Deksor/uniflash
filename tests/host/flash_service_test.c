...
typedef struct mock_flash {
    ...
    uf_bool_t fail_algorithm;
    uf_bool_t toggle_reads;
    uf_bool_t zero_reads;
    uf_bool_t use_default_read_value;
    ...
    uf_bool_t lock_states[4];
} mock_flash_t;

static uf_bool_t mock_read_byte(
    void *context,
...
)
{
    ...
    if (mock->read_script_index < mock->read_script_count) {
        ...
        *value = mock->read_values[mock->read_script_index];
        ...
        address == UINT32_C(0x1F0000)
        && mock->sharp_top_busy_reads > 0
    ) {
...
static uf_bool_t mock_write_byte(
    void *context,
    uf_rom_offset_t address,
    uint8_t value
)
...
static uf_bool_t mock_read_source_byte(
    void *context,
...
)
{
    ...
    if (
        source_address < mock->source_base
        || source_address - mock->source_base >= mock->source_size
    ) {
        return UF_FALSE;
    }
...
static uf_bool_t mock_delay_us(void *context, uint32_t microseconds)
{
    mock_flash_t *mock = (mock_flash_t *)context;
...
static uf_bool_t mock_select_window(
    void *context,
...
static uf_bool_t mock_set_write_lock(
    void *context,
...
)
{
    ...
    return mock->fail_algorithm ? UF_FALSE : UF_TRUE;
}

static uf_bool_t mock_erase(
    struct uf_flash_service *service,
...
static uf_bool_t mock_read_block(
    void *context,
...
static uf_bool_t mock_write_block(
    void *context,
    const void *source,
    uf_rom_offset_t flash_address,
    uf_rom_size_t size_bytes
...
static uf_bool_t mock_compare_block(
    void *context,
    const void *source,
    uf_rom_offset_t flash_address,
    uf_rom_size_t size_bytes,
...
int main(void)
{
    ...
    assert(uf_flash_access_is_valid(NULL) == UF_FALSE);
    assert(uf_flash_access_is_valid(&access) == UF_TRUE);

    access.read_byte = NULL;
    assert(uf_flash_access_is_valid(&access) == UF_FALSE);
    access.read_byte = mock_read_byte;
    ...
    assert(uf_flash_service_select_chip(&service, 0x01, 0xA1) == UF_TRUE);
    ...
    assert(uf_flash_service_select_chip(&service, 0x01, 0x00) == UF_FALSE);
    assert(service.manufacturer == NULL);
    assert(service.chip == NULL);
    assert(service.manufacturer_id == UINT8_C(0xFF));
    assert(service.device_id == UINT8_C(0xFF));
    ...
    assert(uf_flash_service_command(&service, UINT8_C(0x90)) == UF_TRUE);
    ...
    assert(uf_flash_service_command(&service, UINT8_C(0xF0)) == UF_FALSE);
    assert(mock.writes == 1);
    ...
    assert(uf_flash_id_has_odd_parity(UINT8_C(0x00)) == UF_FALSE);
    assert(uf_flash_id_has_odd_parity(UINT8_C(0x01)) == UF_TRUE);
    assert(uf_flash_id_has_odd_parity(UINT8_C(0x03)) == UF_FALSE);
    assert(uf_flash_id_has_odd_parity(UINT8_C(0x7F)) == UF_TRUE);
    assert(uf_flash_id_has_odd_parity(UINT8_C(0xC2)) == UF_TRUE);
    ...
    assert(id_result.valid == UF_TRUE);
    ...
    assert(uf_flash_register_amd_sector_erase(&algorithms));
    assert(
        algorithms.erase[UF_FLASH_ERASE_AMD_SECTOR]
    ...
    assert(uf_flash_register_amd_bulk_erase(&algorithms));
    assert(
        algorithms.erase[UF_FLASH_ERASE_AMD_BULK]
    ...
    assert(uf_flash_register_amd_embedded_erase(&algorithms));
    assert(
        algorithms.erase[UF_FLASH_ERASE_AMD_EMBEDDED]
    ...
    assert(uf_flash_register_generic_algorithms(&algorithms));
    assert(
        algorithms.program[UF_FLASH_PROGRAM_GENERIC_PAGE_BYTE]
        == uf_flash_program_generic_page
    );
    assert(
        algorithms.program[UF_FLASH_PROGRAM_INTEL_SECTOR]
        == uf_flash_program_intel_sector
    );
    assert(algorithms.program[UF_FLASH_PROGRAM_INTEL_SECTOR_U] == NULL);
    assert(
        algorithms.program[UF_FLASH_PROGRAM_AMD_SECTOR]
        == uf_flash_program_amd_sector
    );
    assert(
        algorithms.program[UF_FLASH_PROGRAM_AMD_EMBEDDED]
        == uf_flash_program_amd_embedded
    );
    assert(
        algorithms.program[UF_FLASH_PROGRAM_AMD_FLASH]
        == uf_flash_program_amd_flash
    );
    assert(
        algorithms.erase[UF_FLASH_ERASE_INTEL_SECTOR]
        == uf_flash_erase_intel_sector
    );
    assert(algorithms.erase[UF_FLASH_ERASE_INTEL_SECTOR_U] == NULL);
    assert(
        algorithms.erase[UF_FLASH_ERASE_AMD_FLASH]
    ...
    assert(uf_flash_register_intel_algorithms(&algorithms));
    assert(
        algorithms.program[UF_FLASH_PROGRAM_INTEL_SECTOR_U]
        == uf_flash_program_intel_sector_u
    );
    assert(
        algorithms.erase[UF_FLASH_ERASE_INTEL_SECTOR_U]
    ...
    assert(
        uf_flash_intel_lock_address(&service, UINT32_C(0x10000))
        == UINT32_C(0xFFB90002)
    );
    ...
    assert(mock.lock_states[0] == UF_FALSE);
    assert(mock.lock_states[1] == UF_TRUE);
    ...
    assert(uf_flash_register_sharp_algorithms(&algorithms));
    assert(
        algorithms.program[UF_FLASH_PROGRAM_SHARP_SECTOR]
        == uf_flash_program_sharp_sector
    );
    assert(
        algorithms.erase[UF_FLASH_ERASE_SHARP_SECTOR]
    ...
    mock.use_default_read_value = UF_TRUE;
    ...
    assert(uf_flash_service_command(NULL, UINT8_C(0x90)) == UF_FALSE);
    assert(uf_flash_service_init(NULL, &access, 0) == UF_FALSE);
    assert(uf_flash_service_init(&service, NULL, 0) == UF_FALSE);
    return 0;
}
