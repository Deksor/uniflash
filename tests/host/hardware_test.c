...
static config_entry_t *find_config(mock_hardware_t *mock, uint32_t address) {
    uint8_t index;
...
static void set_config(mock_hardware_t *mock, uf_pci_address_t address, uint8_t reg, uint32_t value) {
...
static uint32_t get_config(mock_hardware_t *mock, uf_pci_address_t address, uint8_t reg) {
    config_entry_t *entry = find_config(mock, uf_pci_mechanism1_address(address, reg));
...
static bool in8(void *context, uf_io_port_t port, uint8_t *value) {
    ...
    *value = port == mock->ct_port ? mock->ct_status : mock->ports[port];
    return true;
}

static bool in16(void *context, uf_io_port_t port, uint16_t *value) {
    mock_hardware_t *mock = context;
    *value = (uint16_t)mock->ports[port] | ((uint16_t)mock->ports[(uint16_t)(port + 1)] << 8);
    return true;
}

static bool in32(void *context, uf_io_port_t port, uint32_t *value) {
    mock_hardware_t *mock = context;
    ...
    if (port == UF_PCI_CONFIG_DATA_PORT) {
        ...
        *value = (uint32_t)mock->ports[port] | ((uint32_t)mock->ports[(uint16_t)(port + 1)] << 8) |
...
static bool out8(void *context, uf_io_port_t port, uint8_t value) {
    mock_hardware_t *mock = context;
...
static bool out16(void *context, uf_io_port_t port, uint16_t value) {
    mock_hardware_t *mock = context;
...
static bool out32(void *context, uf_io_port_t port, uint32_t value) {
    mock_hardware_t *mock = context;
...
static void init_hardware(uf_hardware_t *hardware, mock_hardware_t *mock) {
...
static bool select_bank(void *context, uint8_t bank) {
    mock_hardware_t *mock = context;
...
static void test_pci_address(void) {
    ...
    address.device = 5;
    address.function = 3;
    assert(uf_pci_mechanism1_address(address, UINT8_C(0x4F)) == UINT32_C(0x80022B4C));
}
...
static void test_flash_backend_banking(void) {
    ...
    assert(uf_system_flash_backend_init(&backend, &hardware, UINT32_C(0x1000)));
    assert(uf_system_flash_backend_set_banking(&backend, UINT32_C(0x100), UINT32_C(0x2000), select_bank, &mock));
    assert(uf_system_flash_backend_make_access(&backend, &access));
    mock.physical[UINT32_C(0x1055)] = UINT8_C(0xA5);
    assert(access.read_byte(access.context, UINT32_C(0x255), &value));
    assert(value == UINT8_C(0xA5));
    assert(mock.selected_bank == UINT8_C(2));
    assert(access.write_byte(access.context, UINT32_C(0x355), UINT8_C(0x5A)));
...
static void test_intel_chipset_enable_restore(void) {
    ...
    assert(uf_chipset_rom_set_enabled(&chipset, true));
    assert(get_config(&mock, south, UINT8_C(0x4C)) == UINT32_C(0x12350000));
    assert(get_config(&mock, south, UINT8_C(0xE0)) == UINT32_C(0xFF000000));
    assert(uf_chipset_rom_set_enabled(&chipset, false));
    assert(get_config(&mock, south, UINT8_C(0x4C)) == UINT32_C(0x12340000));
    assert(get_config(&mock, south, UINT8_C(0xE0)) == UINT32_C(0x40000000));
}
...
static void test_ct_flasher_window(void) {
    ...
    assert(uf_ct_flasher_detect(&flasher, &hardware, UINT16_C(0x300), false));
...
static void test_3com_pci_flash_access(void) {
    ...
    assert(uf_pci_rom_backend_set_enabled(&backend, true));
    assert(access.write_byte(access.context, UINT32_C(0x22), UINT8_C(0x5A)));
    ...
    assert(uf_pci_rom_backend_set_enabled(&backend, false));
...
static void test_pci_ids_names(void) {
    ...
    assert(strcmp(uf_pci_rom_device_name_from_ids(&device, "tests/fixtures/pci.ids"),
               "3Com Corporation 3C900 10Mbps Combo [Boomerang]") == 0);
    assert(
        strcmp(uf_pci_rom_device_name_from_ids(&device, "tests/fixtures/missing-pci.ids"), "3Com EtherLink XL") == 0);

    device.pci_device.device_id = 0xFFFF;
    assert(strcmp(uf_pci_rom_device_name_from_ids(&device, "tests/fixtures/pci.ids"), "PCI or AGP card") == 0);
}
