...
static bool in32(void *context, uf_io_port_t port, uint32_t *value)
{
    ...
    {
        *value = (uint32_t)mock->ports[port] | ((uint32_t)mock->ports[(uint16_t)(port + 1)] << 8) | ((uint32_t)mock->ports[(uint16_t)(port + 2)] << 16) | ((uint32_t)mock->ports[(uint16_t)(port + 3)] << 24);
    }
    return true;
}
...
static void test_pci_address(void)
{
    ...
    address.function = 3;
    assert(
        uf_pci_mechanism1_address(address, UINT8_C(0x4F)) == UINT32_C(0x80022B4C));
}
...
static void test_intel_chipset_enable_restore(void)
{
    ...
    assert(
        get_config(&mock, south, UINT8_C(0x4C)) == UINT32_C(0x12350000));
    assert(
        get_config(&mock, south, UINT8_C(0xE0)) == UINT32_C(0xFF000000));
    assert(uf_chipset_rom_set_enabled(&chipset, false));
    assert(
        get_config(&mock, south, UINT8_C(0x4C)) == UINT32_C(0x12340000));
    assert(
        get_config(&mock, south, UINT8_C(0xE0)) == UINT32_C(0x40000000));
}
