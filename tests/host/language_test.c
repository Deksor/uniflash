...
int main(void)
{
    ...
    assert(strcmp(uf_language_get(&uf_language_english, 51),
        "              Flash ROM chip: ") == 0);
    assert(strcmp(uf_language_get(&uf_language_english, 140),
        "Compare backup BIOS image with file") == 0);
    ...
    for (first = 0; first < uf_language_english.message_count; ++first) {
        ...
        for (
            second = first + 1;
            second < uf_language_english.message_count;
            ++second
        ) {
            assert(
                uf_language_english.messages[first].id
                != uf_language_english.messages[second].id
            );
        }
    }
